...
## Phase 1: Tooling and Infrastructure (uv & just) [checkpoint: ec5beee]
...
## Phase 2: Configuration Driven Templates
- [ ] Task: Write failing tests for loading template registry from `config.yaml`.
- [ ] Task: Update `config.yaml` with the detailed template metadata structure.
- [ ] Task: Implement reading template configurations from `config.yaml` in `src/main.py` and `src/engine.py` to make tests pass.
- [ ] Task: Refactor existing code to fully remove the hardcoded `TEMPLATE_REGISTRY`.
- [ ] Task: Conductor - User Manual Verification 'Phase 2: Configuration Driven Templates' (Protocol in workflow.md)
